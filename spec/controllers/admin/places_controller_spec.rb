...
describe Admin::PlacesController do
  before(:each) do
    @user = double(User)
    @user1 = double(User)
    @place = double(Place, :title => "Its a test place")
  end

	describe "Action Activate" do
	  def do_activate
	  	post :activate, :flag => "active", :id => "1"
	  end
      ...
      context "when user as deactivated" do
        before(:each) do
          User.should_receive(:find_by_id).with(1).and_return(@user)
          Place.should_receive(:find_by_id).with("1").and_return(@place)
          @place.stub(:user).and_return(@user1)
          @user1.should_receive(:activated).and_return(false)
        end

        it "should redirect to admin_users_path" do
          do_activate
          flash.alert.should eq("Owner of this place is deactivated. Please activate him first.")
      ...
      context "with current user as admin" do
        before(:each) do
          2.times { Place.should_receive(:find_by_id).with("1").and_return(@place) }
          @place.stub(:user).and_return(@user1)
          @user1.should_receive(:activated).and_return(true)
          User.should_receive(:find_by_id).with(1).and_return(@user)
